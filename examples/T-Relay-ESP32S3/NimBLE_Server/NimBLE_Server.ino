    ...
    */
    void onWrite(NimBLECharacteristic *pCharacteristic)
    {
        ...
        if (pCharacteristic->getValue()) {
            const uint8_t *data = pCharacteristic->getValue().getValue(NULL);
            if (data && pCharacteristic->getValue().length() == 2) {
                Serial.print("Set Pin:"); Serial.print(data[0]); Serial.print(" to "); Serial.println(data[1]);
                control->set(data[0], data[1]);
            }
        }
    };
